...
const monthlyOrderListReducer = (state = initialState, action) =>
  produce(state, draft => {
    switch (action.type) {
      case GET_MONTHLY_ORDER_LIST_SUCCESS:
        let dataPayment = [];
        let dataPaymentNone = [];
        if (action.response) {
        ...
        break;
      case GET_MONTHLY_ORDER_LIST_FAIL:
...
export default monthlyOrderListReducer;
