...
export function ExportBillRoom(props) {
  const [totalkWhMon, setTotalkWhMon] = useState(1);
  const [totalAndTaxAllState, setTotalAndTaxAllState] = useState(1);
  ...
  const updateElectricity = value => {
    // Update electric
    unitPriceElectricityLocal = totalElectricityction;
    const typeElectricity = value;
    const result = unitPriceElectricity * typeElectricity;
    totalElectricityction = result;
  ...
  const downloadFile = async payload => {
    console.log({payload})
    console.log("XuâttttXXXX")
    const requestUrl = urlLink.api.serverUrl + urlLink.api.motelCreateOrderAndPdf;
    try {
  ...
  // const [ dayStartValue, setDayStartValue ] = useState(convertDateFormat(dayStart));
  const [ dayStartValue, setDayStartValue ] = useState();
  const [ dayEndValue, setDayEndValue ] = useState();

  const [ testDay, setTestDay ] = useState(dayStart);

  const handleChangeStart = (evt) => {
    setDayStartValue(evt.target.value);
    console.log("Start", evt.target.value);
    console.log("Start", evt.target);
    // dayStart = evt.target.value;
  }
  const handleChangeEnd = (evt) => {
    setDayEndValue(evt.target.value);
    // dayEnd = evt.target.value;
    console.log("End", evt.target.value);
    console.log("End", evt.target);
  }

  const handleChangeTest = (evt) => {
    setTestDay(evt.target.value);
  }

  const reFilterData = async() => {
    const payload = {
      id,
      idroom,
      idUser,
  ...
  return (
    <div className="login-page-wrapper">
      <Helmet>
        <title>ExportBillRoom</title>
        <meta name="description" content="Description of ExportBillRoom" />
      </Helmet>
          ...
          >
            ...
            <Container>
              ...
              <Row className="infor" style={{ paddingTop: '20px' }}>
                <Col md={3}>
                  {
                    <FormattedMessage {...messages.startFilter}>
                      {msg => (
                        <InputForm
                          label={
                            <FormattedMessage
                              {...messages.startFilter}
                            />
                          }
                          placeholder={msg}
                          name="dayStart"
                ...
                <Col md={3}>
                  {
                    <FormattedMessage {...messages.endFilter}>
                      {msg => (
                        <InputForm
                          label={
                            <FormattedMessage
                  ...
                  <div>
                    <form onSubmit={handleSubmit}>
                      ...
                      <Row>
                        ...
                        <Col md={3}>
                          {
                            <FormattedMessage {...messages.endDay}>
                              {msg => (
                                <InputForm
                                  label={
                                    <FormattedMessage
                            ...
                            <FormattedMessage {...messages.typeElectricity}>
                              {msg => (
                                <InputForm
                                  ...
                                  onBlur={handleBlur}
                                  disabled={idElectricMetter !== null}
                                />
                              )}
                            </FormattedMessage>
                          }
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    handleUpdateTotalElectricity: value => {
      dispatch(updateTotalElectricity(value));
    },
  };
}
...
export default compose(withConnect)(ExportBillRoom);
