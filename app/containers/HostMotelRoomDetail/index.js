...
import makeSelectHostMotelRoomDetail from './selectors';
import './style.scss';

export function HostMotelRoomDetail(props) {
  useInjectReducer({ key: 'hostMotelRoomDetail', reducer });
  useInjectSaga({ key: 'hostMotelRoomDetail', saga });
  const { listRoom = [] } = props.hostMotelRoomDetail;
  const { id = '' } = useParams();
...
HostMotelRoomDetail.propTypes = {
  getListRoom: PropTypes.func,
};

const mapStateToProps = createStructuredSelector({
  hostMotelRoomDetail: makeSelectHostMotelRoomDetail(),
});
...
export default compose(withConnect)(HostMotelRoomDetail);
